 ...
 * @author Henry
 * 
 *         Copy this package to your repo (a package can copied via 1)
 *         highlighting the package folder, 2) right click and then 3) copy). If
 *         that fails, copy the files one by one ;)
 * 
 * 
 ...
 * 
 *         8) Bonus: read the comments of the method:
 *         myCharactersAfterMyArchNemesisHacksIntoMyAccount and implement
...
public class Wow {
	...
	private static void checkAndRemoveAnyHordeSpies(
		...
		while (iterator.hasNext()) {
			...
			if (character.getRace().equals(Race.ORC)) { // TODO not quite right
														// is it?
				iterator.remove();
			}
		}
	...
	private static void printCharacters(ArrayList<Character> characters) {
		Comparator<Character> chars = new Comparator<Character>() {
			...
			public int compare(Character char1, Character char2) {
				int retval = Integer.valueOf(char1.getLevel()).compareTo(
						char2.getLevel());
				if (retval == 0) {
					retval = Integer.valueOf(char1.getStrength()).compareTo(
							char2.getStrength());
					if (retval == 0) {
						retval = Integer.valueOf(char1.getArmour()).compareTo(
								char2.getArmour());
					}
				}
				return retval;
		...
		for (Character character : characters) {
			...
			System.out.println("Level: " + character.getLevel() + ", Strength "
					+ character.getStrength() + ", Armour: "
					+ character.getArmour() + ", Race: " + character.getRace());
		}
	}

	private static void checkForPandaren(ArrayList<Character> characters)
			throws DontHaveTheExpansionException {
		// TODO if there are any PANDAREN races in our List, throw a
		// DontHaveTheExpansionException
		// hint, you should change this method signature
		Iterator<Character> iterator = characters.iterator();
		while (iterator.hasNext()) {
			Character character = iterator.next();
			if (character.getRace().equals(Race.PANDAREN)) { // TODO not quite
																// right is it?
	...
	/**
	 * Bonus Question (2% of overall grade) So here's the scenario: I have 85
	 * characters in my WoW account. Ironically enough, each character's level
	 * is unique i.e. I have a Level1, Level2, Level3 ... Level85 characters
	 * (although in a shuffled order) Then one day, my arch-nemesis hacks into
	 * my account and DELETEs only one of my characters. Write a program that
	 * finds out which character level he DELETEd. Maximum marks will be given
	 * for efficiency. It is possible to solve this problem by looping through
	 * the arraylist just **ONCE** Also note, that although this isn't
	 * necessarily a Java question, this is a permutation of an interview
	 * question used by a few Fortune 100 companies.
	 * 
	 * Its a true *computer science* question Also note that if you can solve
	 * this question, most likely you have a greater computer science mind than
	 * 90% of the instructors at BCIT and you're bored silly of this course
	 * because its too easy for you ;)
	 * 
	 * And just for the record, it took me *OVER* half an hour to solve this.
	 * Its tricky ;)
	 */
